...
1. 参照関係による自動追跡が本番で働かない

   - 対応する要件 ID: `COL-017`、`TRK-004`、`TRK-005`
   - 現状: pure selector は参照方向ごとの追加を実装している `src/domain/tracking-selection.ts:545`。本番の接続生成は authoritative relation だけを native dependency または native sub-issue へ変換し `src/cli/production-runtime.ts:865`、その結果だけを selector へ渡している `src/cli/production-runtime.ts:1107`。
   - 足りない点: cross-reference と本文やコメントの参照を reference connection として本番の selector へ渡す処理がない。

2. 時間経過だけでは severity が進まない

   - 対応する要件 ID: `GOL-005`、`WEB-003`、`NTF-006`
   - 現状: 詳細を再取得しない項目は前回の TrackedItem をそのまま追加する `src/cli/production-runtime.ts:2441`。snapshot の severity も前回値を再利用し `src/cli/production-runtime.ts:2976`、通知評価は今回解析した項目だけを対象にする `src/cli/production-runtime.ts:2998`。
   - 足りない点: 全追跡項目の severity を run 時刻で再計算し、閾値到達を通知候補へ反映する処理がない。

3. team membership が責務判定へ反映されない

   - 対応する要件 ID: `GOL-002`、`CFG-002`、`CFG-003`、`CFG-004`
   - 現状: 本番は repository ごとの team を解決して状態機械へ渡す `src/cli/production-runtime.ts:1320`。状態機械は team node ID の重複だけを検証し `src/domain/issue-state-machine.ts:327`、未アサイン項目には常に maintainer role を設定する `src/domain/issue-state-machine.ts:831`。membership 判定関数は定義されている `src/domain/team-resolution.ts:174` が本番から呼ばれない。
   - 足りない点: author や actor の membership を判定へ使い、maintainer と reviewer の role を設定済み team へ解決する処理がない。

4. Discord 通知自身の失敗を運用障害として検知できない

   - 対応する要件 ID: `NTF-012`、`OPS-002`
   - 現状: `notify-operations` は collect、build、deploy だけに依存し、その失敗だけを条件にする `.github/workflows/daily.yml:254`。CLI の運用障害種別も collection と pages に限定されている `src/cli/command.ts:379`。
   - 足りない点: `notify-discord` の失敗を監視対象へ加え、discord 種別の運用障害通知を送る経路がない。

5. 初回 run の障害を運用通知できない

   - 対応する要件 ID: `NTF-012`
   - 現状: `notify-operations` job は tracker-state branch を無条件で取得する `.github/workflows/daily.yml:274`。CLI も state branch がなければ処理を中止する `src/cli/production-runtime.ts:3998`。
   - 足りない点: 初回 run で state branch がまだ存在しなくても、重複を抑えながら運用障害を通知する経路がない。

6. 失敗 run の report が Actions artifact に残らない

   - 対応する要件 ID: `OPS-003`
   - 現状: CLI は失敗時にも report を書く `src/cli/daily-transaction.ts:484`。workflow の artifact 保存は収集成功後に実行され、validated run だけを対象にする `.github/workflows/daily.yml:111`。
   - 足りない点: 失敗時にも report を artifact へ保存する処理と、後続 job を含む workflow 全体の失敗をまとめる report がない。

7. Pull Request の reviewState と checkState が unknown のままになる
...
8. automation noise の通知抑制が本番で働かない
...
11. state schema の version dispatch と migration がない
...
12. 永続 run report の時間と通知件数が実績を表さない
...
13. Codex の notification recommendation が通知選別へ渡らない
...
14. author と event actor を保存せず検索できない
...
15. relation contradiction の構造が永続化されない
...
16. repository cluster 単位で graph を閲覧できない
...
18. staleness.timezone が表示へ反映されない
