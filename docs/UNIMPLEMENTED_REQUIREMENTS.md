...
1. automation noise の通知抑制が本番で働かない
...
4. state schema の version dispatch と migration がない
...
5. 永続 run report の時間と通知件数が実績を表さない
...
6. Codex の notification recommendation が通知選別へ渡らない
...
7. author と event actor を保存せず検索できない
...
8. relation contradiction の構造が永続化されない
...
9. repository cluster 単位で graph を閲覧できない
...
11. staleness.timezone が表示へ反映されない
