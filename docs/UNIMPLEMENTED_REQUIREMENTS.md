...
3. state schema の version dispatch と migration がない
...
4. 永続 run report の時間と通知件数が実績を表さない
...
5. Codex の notification recommendation が通知選別へ渡らない
...
6. author と event actor を保存せず検索できない
...
7. relation contradiction の構造が永続化されない
...
8. repository cluster 単位で graph を閲覧できない
...
10. staleness.timezone が表示へ反映されない
