...
1. state schema の version dispatch と migration がない
...
2. 永続 run report の時間と通知件数が実績を表さない
...
3. Codex の notification recommendation が通知選別へ渡らない
...
4. author と event actor を保存せず検索できない
...
5. relation contradiction の構造が永続化されない
...
6. repository cluster 単位で graph を閲覧できない
...
8. staleness.timezone が表示へ反映されない
