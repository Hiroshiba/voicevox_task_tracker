...
2. state schema の version dispatch と migration がない
...
3. 永続 run report の時間と通知件数が実績を表さない
...
4. Codex の notification recommendation が通知選別へ渡らない
...
5. author と event actor を保存せず検索できない
...
6. relation contradiction の構造が永続化されない
...
7. repository cluster 単位で graph を閲覧できない
...
9. staleness.timezone が表示へ反映されない
