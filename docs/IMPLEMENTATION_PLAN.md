...
- [x] T18 AI cacheと予算管理 — content-addressed cache、call/文字数/費用上限、予算超過時の優先順位、旧結果の安全再利用
- [x] T19 出力検証とreducer統合 — JSON Schema検証、候補参照とnative relation保護のsemantic検証、AI失敗時のfallback、AI非書込の担保
...
- [x] T22 Web UI基盤と一覧 — Viteビルド、概要dashboard、attention queue、sort/filter可能な表、鮮度表示
- [x] T23 Web UI依存グラフ — component単位のグラフ描画、停滞時間とdownstream impactの視覚強調、frontier/cycle表示、凡例
- [x] T24 Web UI詳細と横断機能 — item詳細、検索、deep link、アクセシビリティ、XSS対策とCSP

### Phase 7: Discord通知

- [x] T25 通知選別 — 通知候補の抽出、noise抑制、ledgerによる重複抑制とcooldown
- [x] T26 digest送信 — payload構成、Discord制限に応じた分割、mention allowlist、空digest抑制、運用障害通知
