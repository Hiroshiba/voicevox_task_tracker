...
- model/prompt更新時、golden evalのcritical/urgent recallを95%以上、誤通知率を10%以下に保つ。
...
5. 変更要求後にauthor push済みなら再review側を評価。
6. 現行review requestがあればrequested user/team待ち。
7. draftは原則author待ち。ただし明示的判断依頼・blockerを優先。
8. 必要承認/checks済みなら`ready_to_merge`＋maintainer待ち。
9. ready-for-reviewでreview未依頼ならmaintainer待ち。
10. CI失敗・コメント意味等が曖昧な場合だけCodexへ渡す。
...
AnalysisMetadata
  deterministicRulesVersion, model, backendVersion
...
| `RSP-015` | MUST | 未解決human thread — 未解決のhuman review threadをactionable signalとして扱わなければならない。                                                                   | `AT-RSP-015`: unresolved human thread fixtureでresolved版よりauthor待ち優先度が高い。      |
...
| `AIC-003` | MUST | content-addressed cache — model、Codex CLI/API version、promptVersion、schemaVersion、normalized input hashをcache keyに含めなければならない。    | `AT-AIC-003`: いずれか1要素変更でcache missになる。                                                   |
| `AIC-004` | MUST | Structured Output — Codex最終出力をrepository内JSON Schemaで拘束しなければならない。                                                              | `AT-AIC-004`: 非JSON・extra property・enum外出力が受理されない。                                      |
...
| `AIC-015` | MUST | 再現情報 — 各AI結果にmodel identifier、backend version、promptVersion、schemaVersion、input/output hash、実行時刻を記録しなければならない。       | `AT-AIC-015`: 任意結果から全再現metadataが取得できる。                                                |
...
| `AIC-019` | MUST | 更新前回帰評価 — model、prompt、schema、判定ルール更新はgolden evalの基準を満たさない限りmainへ反映してはならない。                               | `AT-AIC-019`: 意図的退行PRでCIが失敗する。                                                            |
...
| `DAT-006` | MUST | atomic/canonical/public-safe commit — validation完了後だけsorted/canonical stateをatomic commitし、secret、raw token、private repo ID、不要な全文本文を含めてはならない。 | `AT-DAT-006`: 失敗途中でlast good commitが変わらず、secret scan/private sentinel testが成功する。 |
...
| `SEC-004` | MUST | public-only fail closed — serializationとpublish直前に独立したpublic repo allowlist検証を行い、違反1件でも新state/Pages/Discord公開を中止しなければならない。              | `AT-SEC-004`: private sentinel注入fixtureで3出力すべて停止する。               |
...
- Codex model identifierとcall/token予算
...
- model/prompt/schema/状態判定変更はgolden evalとDiscord snapshot差分をPRでreviewする。
